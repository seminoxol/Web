...
const ownerEmail = ({ name, company, email, phone, productList, dimensions, message }) => emailShell(
    `<h2>New Quote Request</h2><p class="sub">Submitted via pciglass.ca</p><table>${
        [
            ['Name', name],
            company && ['Company', company],
            ['Email', `<a href="mailto:${email}" style="color:#2EC4B6">${email}</a>`],
            ['Phone', `<a href="tel:${phone}" style="color:#2EC4B6">${phone}</a>`],
            ['Products', productList],
            dimensions && ['Dimensions', dimensions],
            message && ['Message', `<span style="white-space:pre-line">${message}</span>`]
        ].filter(Boolean).map(row).join('')
    }</table><p class="note">Reply directly to this email to respond to ${name}.</p>`
);
...
module.exports = { ownerEmail, customerEmail };
