...
const buildHeader = ({ activePage = null, solid = false } = {}) => {
    ...
    const productsClass = (activePage === 'glass' || activePage === 'windows') ? ' nav__link--active' : '';
    const productsAria = (activePage === 'glass' || activePage === 'windows') ? ' aria-current="page"' : '';

    return headerTemplate
        .replace('{{HEADER_CLASS}}', headerClass)
        .replace('{{NAV_FAQ_CLASS}}', faqClass)
        .replace('{{NAV_FAQ_ARIA}}', faqAria)
        .replace('{{NAV_PRODUCTS_CLASS}}', productsClass)
        .replace('{{NAV_PRODUCTS_ARIA}}', productsAria);
};
