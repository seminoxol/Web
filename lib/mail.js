...
const resendFrom = () => {
    const raw = clean(process.env.RESEND_FROM) || clean(process.env.EMAIL_FROM);
    if (raw && !RESEND_SANDBOX_FROM.test(raw)) return raw;
    if (raw && RESEND_SANDBOX_FROM.test(raw)) {
        console.warn('RESEND_FROM is onboarding@resend.dev — using verified domain sender instead.');
    }
...
const initMail = async () => {
    if (hasResend()) {
        ...
        const sandbox = RESEND_SANDBOX_FROM.test(clean(process.env.RESEND_FROM) || clean(process.env.EMAIL_FROM) || '');
        if (sandbox) {
            console.warn(`RESEND_FROM was sandbox address — using ${from} instead. Update Render env to quotes@pciglass.ca.`);
        }
        status = { ready: true, provider: 'resend', verified: true, error: null, from };
        console.log(`✉️  Resend ready — from ${from} → ${ownerAddress()}`);
        return status;
    }
...
const sendQuoteEmails = async payload => {
    ...
    if (ownerSent) lastSendError = null;
...
const getMailStatus = () => ({
    ...
    sendFrom: hasResend() ? resendFrom() : null,
    lastSendError
});
